...
export default function EditProfileScreen({ navigation }) {
  ...
  const [bloodType, setBloodType] = useState<UpdateUserPayload["blood_type"]>(user?.blood_type);
  const { colors } = useTheme();

  const handleSave = async () => {
    console.log("Saving profile with:", { name, bloodType });
    if (!name.trim()) {
      Alert.alert("Помилка", "Ім'я не може бути порожнім");
      return;
    }
    try {
      await updateUserAction({ name, blood_type: bloodType });
