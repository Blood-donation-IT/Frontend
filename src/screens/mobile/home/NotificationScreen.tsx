...
const NotificationScreen = () => {
  ...
  i18n.language === "uk" ? uk : enUS;
  const notifications = [
    {
      id: "1",
      titleKey: "donation_reminder_title",
      messageKey: "donation_reminder",
      date: createDateWithOffset(0, 10, 0),
    },
    {
      id: "2",
      titleKey: null,
      messageKey: "donation_reminder",
      date: createDateWithOffset(-15, 16, 20),
    },
    {
      id: "3",
      titleKey: "account_setup_success",
      messageKey: null,
      date: createDateWithOffset(-365, 14, 40),
    },
  ];
  ...
  const groupedNotifications = useMemo(() => {
    ...
    sorted.forEach((item) => {
      ...
      else if (diff < 7) title = t("daysAgo", { count: diff });
      else if (diff < 365)
        title = t("weeksAgo", { count: Math.floor(diff / 7) });
      else title = t("yearsAgo", { count: Math.floor(diff / 365) });
              ...
              return (
                <View key={item.id} style={styles.card}>
                  <View style={styles.cardContent}>
                    {item.titleKey && (
                      <Text style={styles.cardText}>{t(item.titleKey)}</Text>
                    )}
                    {item.messageKey && (
